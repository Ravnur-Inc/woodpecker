...
To stay within this limit, the `{{ .event }}` part of the status key is shortened to `pr` for `pull_request` and to `pr_comment` for `pull_request_comment` pipelines (e.g. `ci/woodpecker/pr_comment/codereview`).
